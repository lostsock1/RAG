...
def test_ppq_backend_shapes_messages_with_context_in_stable_order() -> None:
    ...
    assert user_message.count("citation_id=c1") == 1
    assert user_message.count("citation_id=c2") == 1
    assert user_message.index("citation_id=c1") < user_message.index("citation_id=c2")
    assert "document_title=Doc A" in user_message
    assert "document_title=Doc B" in user_message
    assert "text=Alpha" in user_message
    assert "text=Beta" in user_message
