...
> **F2.1 landed 2026-06-13** — profile selection at upload + jobs API + profile-routed chunking. `profile: loose|book` (default loose) is snapshotted on the **IngestionRun** (migration `20260613_0011`, server_default backfills loose) and surfaced in the upload response + jobs list/detail; `run_chunk_stage` now routes off the **persisted** run profile instead of the old `source_type != "loose_document"` guess (defensive unknown→loose coerce). Loose path byte-identical → eval baseline preserved. OpenAPI updated; +6 tests; suite 605 non-slow.
> **F2.4 landed 2026-06-13** — textbook **PDF** page-anchor e2e. Committed a digital-born 2-page music-theory PDF (`tests/fixtures/textbook_excerpt.pdf`, span-isolation-safe subject), authored with the already-present PyMuPDF (no new project dep); new `slow` e2e parses it with REAL Docling → `BookDocumentChunker` and proves per-item page anchors flow into chunk `page_start`/`page_end` (Ch1→p1, Ch2→p2 — the guarantee pageless Markdown can't give). **Real-Docling-PDF finding pinned:** the layout model tags every heading `section_header` at `level=1` (no font-size→depth), so PDF heading_paths are flat (one section per heading), unlike the Markdown backend's nested chapter→section breadcrumbs.
> **Remaining F2**: book eval fixtures + textbook heldout activation (F2.2); multi-hop heldout subset → ADR-0021 reopen trigger 2 (F2.3).
