...
- [ADR-0015 — Eval Harness Implementation](adr/0015-eval-harness-implementation.md) — Accepted 2026-05-23
...
- [ADR-0021 — Query Understanding: Route-Gated Multi-Query + Heuristic Decomposition](adr/0021-query-understanding.md) — Accepted with data 2026-06-12 (rule frozen 2026-06-11 pre-measurement, applied mechanically: multi_query ranking flat |Δ| ≤ 0.0015 with +3.0 s added P50 vs the 700 ms bar despite 60/60 candidate pools genuinely perturbed; decompose triggered 1/60 — its lone firing fully fixed h49 (MRR@10 0.5→1.0), the entire +0.0084 aggregate — subset-honesty reopen evidence, not a pass; `both` lost the h49 fix to RRF paraphrase dilution; default stays `disabled`, all arms config-selectable)
