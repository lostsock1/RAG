...
- `POST /api/v1/chat/stream` emits an evidence-safe SSE sequence with **sentence-incremental verification** (ADR-0018). Generated tokens are assembled into sentences server-side; each completed sentence is verified against the authorized context **before** its text is emitted as a `token` event. A `token` event's `text` is therefore one verified sentence (sentence-granular chunks, not raw LLM tokens). The invariant is unchanged and finer-grained than the earlier buffered slice: **no `token` event is ever emitted before its sentence passes verification.**
- Streaming SSE grammar by outcome (`stream_verification_policy` config, default `retract`):
  - Answered: `retrieval` → `token`+ → `verification` (`supported`) → `citations` → `final` (`answered`) → `done`.
  - No evidence retrieved: `retrieval` → `final` (`not_enough_evidence`) → `done` (LLM never called).
  - First sentence fails verification (nothing emitted): `retrieval` → `verification` (`unsupported`) → `final` (`not_enough_evidence`) → `done`.
  - Mid-stream failure, `retract` policy: `retrieval` → `token`+ → `verification` (`unsupported`) → `retraction` (`{"reason": "verification_failed"}`) → `final` (`not_enough_evidence`) → `done`. Clients **must** replace any displayed partial text with the final message on `retraction`.
  - Mid-stream failure, `truncate` policy: `retrieval` → `token`+ → `verification` (`unsupported`) → `citations` → `final` (`answered`, `"truncated": true`) → `done`; `final.answer_text` equals exactly the concatenation of emitted `token` texts.
- Streaming strictness note: the streaming path gates **every** sentence individually; the blocking `/chat` path accepts answers with up to `nli_unsupported_ratio` unsupported sentences (ADR-0016). Borderline answers can therefore be answered by `/chat` and retracted by `/chat/stream` — streaming errs stricter by design (ADR-0018).
