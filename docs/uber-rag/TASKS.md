...
- [~] E2 — ADR-0020 + contextual chunk augmentation (breadcrumb + LLM arms via existing seams, bake-off). **Unblocked 2026-06-11** (judge on nDCG/MRR lift vs MRR@10 0.834 / nDCG@10 0.875); **foundation landed 2026-06-11, handed over**: entry-gate sources verified (Anthropic contextual-retrieval numbers + Jina late-chunking as rejected alternative), LLM arm calibrated (313 leaf chunks × ~3 s ppq ≈ 16 min), `chunks.context_prefix` migration + `Chunk.search_text` + `contextualizers/` package (breadcrumb/llm/stub) + optional `contextualize` stage (pipeline stays 7 stages when disabled — bit-identical, suite 511 passed) + OpenSearch `text`=augmented/`display_text`=original + `contextual_augmentation` setting. Remaining (see HANDOVER.md): ADR-0020 with frozen ranking-lift rule BEFORE measuring, TDD tests for the new paths, Settings→contextualizer runtime wiring (truthful failure for llm w/o creds), 3-arm bake-off with positive control, docs.
