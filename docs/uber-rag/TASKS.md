...
- [ ] Distractor corpus (prerequisite for further ranking/recall evals — C5 caveat): near-duplicate/same-topic fixture docs + evidence spans (C5 pattern: `tests/eval/fixtures/sample_corpus/` + `docs/uber-rag/eval/heldout-v1.yaml` evidence blocks, rot-guarded loader asserts).
- [ ] E2 — ADR-0020 + contextual chunk augmentation (breadcrumb + LLM arms via existing seams, bake-off). Blocked on harder distractor corpus or nDCG/MRR-based judging (C5 caveat).
