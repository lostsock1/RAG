...
- [x] Reranker arm (pre-E2, models-freeze compatible): measure `bge-reranker-v2-m3` (accepted ADR-0014 model, config-off in production) on the C3 rig — done 2026-06-11. Prerequisite fix: `BgeRerankerV2M3` reimplemented on plain transformers (FlagEmbedding 1.4.0 `prepare_for_model` removed in transformers 5.x — real path crashed; regression guard added; suite 511 passed). Frozen rule applied to `tests/eval/reports/retrieval_reranker_arm.json`: MRR@10 +0.0132 / nDCG@10 +0.0109 (below +0.02) and mean overhead 2436 ms/query (bar 1000, dev-Mac CPU) — **NO FLIP, `reranker_backend` stays `disabled`**; reopen paths in ADR-0014 (distractor corpus / ONNX serving / smaller candidate pool).
- [x] Distractor corpus (prerequisite for further ranking/recall evals — C5 caveat) — done 2026-06-11. 8 same-topic hard-negative docs (EN×6, DE, PT) under `tests/eval/fixtures/sample_corpus/`: echo each target query's exact subject phrase, state a sibling fact, contain no evidence span (verified across all 60). Re-baselined: **MRR@10 0.927→0.834, nDCG@10 0.944→0.875** (recall@10 stays 1.000 — ranking headroom, not recall, since the measured weakness was ranking). Reranker arm re-run → **quality_pass TRUE** (MRR +0.0413 / nDCG +0.0314, both > +0.02); still NO FLIP on latency (2222 ms). Authored as unlabeled hard negatives (no new heldout questions) — the IR-correct form; recall headroom would need far higher distractor density (deferred, recall was never the weakness).
- [x] E2 — ADR-0020 + contextual chunk augmentation (breadcrumb + LLM arms via existing seams, bake-off) — **done 2026-06-11, ADR-0020 Accepted with data: NO WIN, `contextual_augmentation` stays `disabled`**. Foundation (migration + `search_text` + contextualizers + optional stage, disabled path bit-identical) landed earlier same day; this session: ADR-0020 committed with the rule frozen BEFORE measurement (MRR@10 or nDCG@10 ≥ +0.02, recall@10 drop ≤ 0.02, cost acknowledged, breadcrumb wins ties), 30 TDD tests (suite 541), Settings wiring for in-process + Temporal paths with truthful llm-creds failure (suite 549), 3-arm bake-off on isolated re-ingested stacks with positive control 313/313: **breadcrumb +0.0090 MRR@10 (sub-bar, ~free); llm −0.0867 MRR@10 (actively harmful — topic-level context pulls same-topic distractors closer; 4.56 s/leaf ppq)**. Report `tests/eval/reports/retrieval_contextual_augmentation.json`; reopen triggers incl. real-BM25 rig (dense-only caveat) and book-profile corpora in ADR-0020.
- [x] E3 — ADR-0021 + query understanding (multi-query + decomposition, route-gated; existing LLM seam) — **done 2026-06-12, ADR-0021 Accepted with data: NO WIN (all three arms), `query_understanding` stays `disabled`**. Rule frozen BEFORE measurement (MRR@10 or nDCG@10 ≥ +0.02, recall@10 drop ≤ 0.02, added gated-route P50 ≤ 700 ms; subset wins record-only; decompose zero-trigger = not_exercised). Seams (`query_understanding.py`: protocol + LlmMultiQueryExpander + HeuristicQueryDecomposer + Composite + stub; retriever merges original+expansions through existing RRF, reranks against ORIGINAL query, exact/quoted routes untouched, None-path byte-identical) + 26 unit tests (suite 575) + Settings wiring with truthful no-creds startup failure (suite 580). Bake-off on the session eval stack with paired control, rig equivalence verified per-question: **multi_query dead flat (−0.0012 MRR@10) at +3030 ms added P50 — 4.3× the bar, with positive control proving 60/60 pools genuinely perturbed (recall saturated at 1.000; paraphrases retrieve the same C5 confusables); decompose 1/60 trigger (0/5 multi_hop matched — heldout trigger-shape TODO), lone firing fully fixed h49 (0.5→1.0 = the whole +0.0084); both +2735 ms and LOST the h49 fix to RRF paraphrase dilution.** Report `tests/eval/reports/retrieval_query_understanding.json`; reopen paths in ADR-0021 (local low-latency serving; multi-hop heldout additions — with positive per-trigger evidence; E2-reopen rig upgrades; baseline shift).
...
- [x] F1 — book profile chunker — **done 2026-06-13** (suite **610**). 1/2: `BookDocumentChunker` (F0 `page.blocks` → section-parent→leaf tree, heading-path breadcrumbs, page anchors, atomic tables) + `build_chunker(profile)` factory wired into `run_chunk_stage` (`loose.py` untouched); table blocks carry markdown; 10 unit tests. 2/2: **`persist_chunks` generalized to multi-parent** — keys the parent_id→DB-id map on each parent's chunker-assigned `id` (loose now sets its parent `id`; single-parent guard replaced by a parents-with-children-must-carry-id guard; 5 existing persist tests migrated); 2 new multi-parent persist tests (+guard); 1 `slow` real-Docling→book e2e (Markdown: 3 sections→3 parents + atomic table leaf + breadcrumbs). Page anchors unit-proven (Markdown pageless); real textbook **PDF** page-anchor e2e deferred to F2.
- [ ] F2 — profile selection at upload + profile-aware eval (incl. a real multi-hop heldout subset → fires ADR-0021 reopen trigger 2).
  - [x] F2.1 — document profile selection at upload + jobs API + profile-routed chunking — **done 2026-06-13** (suite **605** non-slow, +6). Upload takes `profile: loose|book` (default loose), snapshotted on `IngestionRun` (migration `20260613_0011`, server_default backfills loose) + surfaced in the upload response + jobs list/detail. `run_chunk_stage` now routes off the **persisted** run profile (was a `source_type != "loose_document"` guess) — `pipeline_runner` reads `claimed_run.profile`; defensive unknown→loose coerce. Loose path byte-identical → committed eval baseline preserved. OpenAPI updated (upload form + `DocumentUploadResponse` + `IngestionJob`). Tests: 3 stage-routing + 2 jobs-API integration + 1 repo.
  - [ ] F2.2 — ≥2 book eval fixtures + activate the textbook heldout subset against book-profile chunks (numbers recorded; span-isolation invariant).
  - [ ] F2.3 — author a real multi-hop heldout subset (MultiHop-RAG/MuSiQue/HotpotQA/2Wiki shapes) → re-run the ADR-0021 decompose arm (reopen trigger 2; heuristic, LLM-free).
  - [x] F2.4 — textbook **PDF** fixture for true page-anchor e2e — **done 2026-06-13**. Committed a digital-born 2-page music-theory PDF (`apps/api/app/tests/fixtures/textbook_excerpt.pdf` + `generate_textbook_pdf.py`), authored with the already-present PyMuPDF (no new project dep; the test reads the committed PDF, needs only Docling). New `slow` e2e: real `DocumentConverter().convert()` → `BookDocumentChunker` proves page anchors flow into `page_start`/`page_end` (Ch1→page 1, Ch2→page 2). Pinned real-Docling-PDF finding: every heading is `section_header` `level=1` (no font-size→depth) → flat heading_paths, unlike Markdown's nested breadcrumbs. Subject disjoint from heldout subjects (span-isolation-safe).
