...
- [~] F1 — book profile chunker. **Increment 1/2 done 2026-06-13** (suite 607): `BookDocumentChunker` (consumes F0 `page.blocks` → section-parent→leaf tree, full heading-path breadcrumbs, page anchors, atomic tables) + `build_chunker(profile)` factory wired into `run_chunk_stage` (`loose.py` untouched); adapter tweak so table blocks carry markdown; 10 unit tests. **Increment 2/2 next**: generalize `persist_chunks` to multi-parent (key the parent_id→DB-id map on each parent's chunker-assigned `id`; set loose's parent `id` for consistency; update the persist tests that build parents with `id=None`), then a real-Docling **digital-born textbook PDF** e2e fixture (chunk→persist→retrieve), span-isolation-safe.
