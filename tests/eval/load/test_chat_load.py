...
async def test_streaming_load_concurrent(eval_stack: EvalStack, monkeypatch):
    ...
    # Measured baseline: P50 ~2.5s, P95 ~3.5s (typical) with ppq.ai + NLI verifier.
    # P95 can spike to ~9s on a single slow ppq.ai request (queue delay).
    ...
    if first_token_values:
        ...
        assert p95_ft < 10000, f"P95 first-token latency {p95_ft:.0f}ms exceeds 10000ms (ADR-0017)"
